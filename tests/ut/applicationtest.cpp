...
TEST_CASE("Application vcs manager is present when the feature is enabled",
          "[Application]")
{
    aide::Application::disableLoggingToConsole();
    ...
    REQUIRE(app.vcsManager().has_value());
}

TEST_CASE("Application vcs manager is empty when the feature is disabled",
          "[Application]")
{
    aide::Application::disableLoggingToConsole();
    ...
    REQUIRE_FALSE(app.vcsManager().has_value());
}
