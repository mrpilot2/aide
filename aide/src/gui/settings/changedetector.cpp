...
namespace
{
    void installChangeDetectorForItemViews(
        ...
        const QObject* child)
    {
        if (const auto* itemView =
                qobject_cast<const QAbstractItemView*>(child);
            itemView != nullptr) {
            if (const auto* itemModel = itemView->model();
                itemModel != nullptr) {
                QObject::connect(itemModel,
                                 &QAbstractItemModel::columnsInserted,
                                 settingsController,
                                 &aide::gui::SettingsDialogController::
                                     onUserChangedAGuiElement);
                QObject::connect(itemModel, &QAbstractItemModel::rowsInserted,
                                 settingsController,
                                 &aide::gui::SettingsDialogController::
                                     onUserChangedAGuiElement);
                QObject::connect(itemModel, &QAbstractItemModel::columnsMoved,
                                 settingsController,
                                 &aide::gui::SettingsDialogController::
                                     onUserChangedAGuiElement);
                QObject::connect(itemModel, &QAbstractItemModel::rowsMoved,
                                 settingsController,
                                 &aide::gui::SettingsDialogController::
                                     onUserChangedAGuiElement);
                QObject::connect(itemModel, &QAbstractItemModel::columnsRemoved,
                                 settingsController,
                                 &aide::gui::SettingsDialogController::
                                     onUserChangedAGuiElement);
                QObject::connect(itemModel, &QAbstractItemModel::rowsRemoved,
                                 settingsController,
                                 &aide::gui::SettingsDialogController::
                                     onUserChangedAGuiElement);
                QObject::connect(itemModel, &QAbstractItemModel::dataChanged,
                                 settingsController,
                                 &aide::gui::SettingsDialogController::
                                     onUserChangedAGuiElement);
            }
        ...
        const QObject* child)
    {
        if (const auto* keyEdit = qobject_cast<const QKeySequenceEdit*>(child);
            keyEdit != nullptr) {
            QObject::connect(
                keyEdit, &QKeySequenceEdit::keySequenceChanged,
                settingsController,
                &aide::gui::SettingsDialogController::onUserChangedAGuiElement);
        }
    }
    ...
    void installChangeDetectorsForDateTimeEdits(
        ...
        const QObject* child)
    {
        if (const auto* dateTimeEdit =
                qobject_cast<const QDateTimeEdit*>(child);
            ...
            QObject::connect(
                ...
                &aide::gui::SettingsDialogController::onUserChangedAGuiElement);
            QObject::connect(
                dateTimeEdit, &QDateTimeEdit::dateTimeChanged,
                settingsController,
                &aide::gui::SettingsDialogController::onUserChangedAGuiElement);
        }
    }
